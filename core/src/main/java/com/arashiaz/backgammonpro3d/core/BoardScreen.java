...
public final class BoardScreen extends ScreenAdapter implements InputProcessor {
    ...
    private Material mat(float r,float g,float b,float roughness) {
        ...
        m.set(FloatAttribute.createShininess(24f*(1f-roughness)+4f)); return m;
    }
    private void buildBoard() {
        long attrs=VertexAttributes.Usage.Position|VertexAttributes.Usage.Normal;
        boardModel=mb.createBox(18f,0.65f,10f,mat(0.30f,0.13f,0.055f,0.35f),attrs); models.add(new ModelInstance(boardModel,0,-0.35f,0));
        Model inner=mb.createBox(16.9f,0.42f,9.05f,mat(0.60f,0.31f,0.10f,0.42f),attrs); models.add(new ModelInstance(inner,0,0.05f,0));
        pointModel=mb.createCone(1.18f,0.22f,4.0f,3,mat(0.87f,0.68f,0.42f,0.55f),attrs);
        for(int i=0;i<12;i++){float x=-7.25f+i*1.32f; ModelInstance a=new ModelInstance(pointModel,x,0.34f,-2.25f); a.transform.rotate(Vector3.X,90); models.add(a); ModelInstance b=new ModelInstance(pointModel,x,0.34f,2.25f); b.transform.rotate(Vector3.X,-90); models.add(b);}
        darkChecker=mb.createCylinder(1f,0.38f,1f,48,mat(0.08f,0.075f,0.085f,0.15f),attrs);
        lightChecker=mb.createCylinder(1f,0.38f,1f,48,mat(0.90f,0.84f,0.70f,0.18f),attrs);
        ...
        diceModel=mb.createBox(1.35f,1.35f,1.35f,mat(0.94f,0.94f,0.91f,0.12f),attrs); models.add(new ModelInstance(diceModel,-2f,0.85f,0)); models.add(new ModelInstance(diceModel,2f,0.85f,0));
    }
    ...
    @Override public void dispose(){batch.dispose();boardModel.dispose();pointModel.dispose();darkChecker.dispose();lightChecker.dispose();diceModel.dispose();}
}